...
- [x] T008 [P] [US3] Compute `category_sales` DataFrame in `app.py`: `df.groupby('category')['total_amount'].sum().reset_index()`, rename to `category` and `total_sales`, sort descending by `total_sales` (data-model.md Category Aggregate; FR-006)
